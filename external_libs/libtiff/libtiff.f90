!>  \brief  Define interfaces to libtiff functions
module libtiff
    implicit none
...
interface
    ...
    !
    ! Functions defined in libtiff itself
    !
    ...
    function TIFFMyOpen(cfilename,cmode) bind(c,name='TIFFMyOpen')
        use iso_c_binding, only : c_ptr, c_char
        type(c_ptr) ::  TIFFMyOpen
        character(kind=c_char)  ::  cfilename(*)
        character(kind=c_char)  ::  cmode(*)
    end function TIFFMyOpen
